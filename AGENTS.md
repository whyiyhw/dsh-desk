...
- **tsx 启动 + profile 插件 = src/lib 混合加载，工具调用全灭（2026-09-21 实锤）**：`node --import tsx/esm apps/cli/src/bin.ts` 让 CLI 侧 import 图把 workspace 包解析到 **src/index.ts**（tsx 满足 source 条件），而 profile 插件链不走 tsx 解析到 **lib/index.js**——同一包双实例入内存，`unique symbol` 身份断裂（`TOOL_RUNTIME_SCHEDULER` 两个同名 symbol），`ctx.tools[sym]` 为 undefined，**每个工具调用**都抛 `Cannot read properties of undefined (reading 'prepare')`，GUI 表现为"本轮运行失败"。诊断配方：headless 复现（`dsh --profile headless "用 skill 工具加载 X"`）+ 在构建产物 lib/index.js 的外层 catch 打 `console.error(error)` 拿堆栈 + `import.meta.resolve('@deepseek-ai/dsh-tools')` 看 CLI 侧解析到 src 还是 lib。**修复：启动命令改 `node apps/cli/lib/bin.js`（全 lib 归一）**——config.json 已改（备份 `D:	mp\desk-config-backup-tsx.json`）；代价=pull 后必须 `pnpm build`（与既有规则一致）。安装版 dsh 全 lib 天然无此病。
- **同日第二坑（profile 级本地插件）**：harness 0.1.6-alpha.2 重构了 client 插件契约（新 boot/模块系统 `__DSH_BOOT__` + ClientModuleSystem、slots API、typert），旧本地插件（如 dsh-zcode-usage 0.1.0，9月4日构建）web 激活失败时**整个 GUI 被启动兜底横幅卡死**（"Failed to load plugins"），且激活错误**不走 console**（进 cordis logger，浏览器侧抓不到）。解锁 = 从 `~/.dsh/profiles/web/package.json` 的 `dsh.profile.bundles` 暂时移除该插件（保存即热生效，无需重启服务器）；正式修复 = 把插件移植到新契约（**已完成，dsh-zcode-usage c4d67df，2026-09-20**）。移植沉淀的四条生命周期规则（详见该仓库 src/client/index.ts 头注）：typert codec 用 `create()` 惰性工厂；命名空间 mount 必须挂 **root 拥有的 fiber**（loader 启动期重建 profile 条目 fiber，会把条目上的 mount 连带拆掉、方法被剥）；`remote.<ns>` 服务只能从**声明了注入的 ctx** 读（代理未注入访问直接抛、`ctx.get` 跨 fiber 分支不可见）——用一次性子 fiber 捕获到闭包格子；slot 注册必须在 apply 内**同步**完成（shell.overlay 插座首渲只画一次，晚到的注册永远不显示）。诊断链沉淀：`window.__DSH_BOOT__.entries` 是清单真源（id/url/rev/inject）；单条目 URL `/plugins/??<pkg>/client.js&rev=<rev>` 可 curl（**旧 rev 重建后 404 是失效机制，不是故障**）；curl `-o /dev/null` 报写错误时 `-w` 的 http_code 不可信（曾把 404 读成 200）。**排查客户端插件首选"仪器化自己的代码"**（把错误写 `window.__x` 数组，页面里 evaluate 读回）——console 钩子永远输给启动竞速，React 错误栈探针还会裸调组件（解构 props 的组件会抛但被 React 自身 try/catch 吞掉，别被这个假象带偏）。
...
- **tao 的 visible 状态与外部 ShowWindow 脱钩**：外部 SW_HIDE 藏窗后调 `window.show()` 同值短路成 no-op（旧插件回调同样性质）——测"弹回唤醒"必须走产品藏窗路径（PostMessage WM_CLOSE → CloseRequested → hide()），别用外部藏窗测。
- **CopyFromScreen 是屏幕区域不是窗口**：覆盖窗（Umi-OCR 主窗）在目标位会截到别人的内容；SetForegroundWindow 从后台进程偷焦点会被 Windows 拒——先最小化覆盖窗。**PS1 无 BOM 时 em dash（U+2014）按 ANSI 读成乱码**（BOM 规则再+1 实例）：脚本里写 `[char]0x2014`，别写字面量。
